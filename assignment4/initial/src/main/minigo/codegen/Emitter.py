...
class Emitter():
    ...
    def getJVMType(self, inType):
        ...
        if typeIn is BoolType:
            ...
            return reduce(lambda acc, ele: acc + '[', inType.dimens, '') + self.getJVMType(inType.eleType)
    ...
    *   @param isFinal true in case of constant; false otherwise
    ...
    def emitATTRIBUTE(self, lexeme, in_, isStatic, isFinal, value):
        ...
        if isStatic:
            ...
            return '.field public ' + lexeme + ' ' + self.getJVMType(in_) + self.jvm.END
    ...
    *   @param isStatic <code>true</code> if the method is static; <code>false</code> otherwise.
    '''
    def emitAMETHOD(self, lexeme, in_, frame):
        return '.method public abstract ' + lexeme + self.getJVMType(in_) + self.jvm.END + '.end method' + self.jvm.END
    ...
    *   @param in the code to be printed out
    ...
    def emitNEWARRAY(self, in_ ,frame):
        ...
        elif type(in_) is Id or type(in_) is cgen.ClassType:
            return self.emitANEWARRAY(in_, frame)
    ...
    def emitMULTIANEWARRAY(self, in_, dimension ,frame):
        return self.jvm.emitMULTIANEWARRAY(self.getJVMType(in_), str(dimension))
    ...
    def emitINVOKEINTERFACE(self, lexeme, in_, frame):
        ...
        if not type(typ) is VoidType:
            frame.push()
        # return self.jvm.emitINVOKEVIRTUAL(lexeme, self.getJVMType(in_))
        return self.jvm.INDENT + 'invokeinterface ' + lexeme + self.getJVMType(in_) + ' ' + str(len(in_.partype) + 1) + self.jvm.END
    ...
    def emitIMPLEMENT(self, lexeme):
        return '.implements ' + lexeme + self.jvm.END*2
