...
class CodeGenerator(BaseVisitor,Utils):
    ...
    def visitProgram(self, ast: Program, c):
        ...
        for decl in ast.decl:
            if isinstance(decl, FuncDecl): self.visit(decl, sup)
            elif isinstance(decl, VarDecl): 
                sym = self.lookup(decl.varName, env['env'][0], lambda x : x.name)
                sup['env'][0] += [sym]
            elif isinstance(decl, ConstDecl):
                sym = self.lookup(decl.conName, env['env'][0], lambda x : x.name)
                sup['env'][0] += [sym]
